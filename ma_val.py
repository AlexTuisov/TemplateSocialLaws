...
DFILE_KEYWORDS = ["requirements", "types", "predicates",
                  "action", "private", "functions", "constants"]
...
class Predicate(object):
    ...
    def __init__(self, name, args: list, is_typed, is_negated):
        self.name = name
        self.args = args
        self.arity = len(args)
...
class PlanningProblem(object):
    ...
    def get_goal(self, agent):
        ...
        for p in self.goal:
            if p.args[0] in self.agents:
                if p.args[0] == agent:
                    g.append(p)
            # elif p.args[0] in self.objects['truck']:
            #    if p.args[0] in trucks[agent]:
            #         g.append(p)
            # elif p.args[0] in self.objects['package']:
            #    if p.args[0] in packages[agent]:
            #         g.append(p)
            else:
    ...
    def validate_rational(self):
        ...
        for a in self.agents:
            pp.actions.append(Action("end_s_" + a, Predicate('', [["?alocal", "agent"]], True, False),
                                     [Predicate(p.name + "-l", [a] + p.args, p.is_typed, p.is_negated) for p in
                                      self.get_goal(a)] +
                                     [Predicate(
                                         p.name + "-g", p.args, p.is_typed, p.is_negated) for p in self.get_goal(a)],
            ...
            for i, f in enumerate(self.get_goal(a)):
                pp.actions.append(Action("end_f_" + a + "_" + str(i), Predicate('', [["?alocal", "agent"]], True, False),
                                         [Predicate(p.name + "-l", [a] + p.args, p.is_typed, p.is_negated) for p in
                                          self.get_goal(a)] +
                                         [Predicate(
                                             f.name + "-g", f.args, f.is_typed, not f.is_negated)],
                                         [Predicate("fin", [a], False, False), Predicate("act", [], False, True),
        ...
        for a in self.actions:
            a_s = Action(a.name + "_s", a.parameters,
                         ...
                         [Predicate(p.name + "-l", [a.parameters.args[0][0]] + p.args, p.is_typed, p.is_negated) for p in
                          a.precondition] +
                         [Predicate(p.name + "-g", p.args, p.is_typed,
                                    p.is_negated) for p in a.precondition],
                         [Predicate(p.name + "-l", [a.parameters.args[0][0]] + p.args, p.is_typed, p.is_negated) for p in
                          a.effect] +
            ...
            for i, f in enumerate(a.precondition):
                a_f.append(Action(a.name + "_f_" + str(i), a.parameters,
                                  [Predicate("act", [], False, False)] +
                                  [Predicate(p.name + "-l", [a.parameters.args[0][0]] + p.args, p.is_typed, p.is_negated) for p
                                   in a.precondition] +
                                  [Predicate(f.name + "-g", f.args,
                                             f.is_typed, not f.is_negated)],
                                  [Predicate(p.name + "-l", [a.parameters.args[0][0]] + p.args, p.is_typed, p.is_negated) for p
                                   in a.effect] +
                                  [Predicate("failure", [], False, False)]
                                  ))
            ...
            pp.init = [act]
            for f in self.init:
                pp.init.append(Predicate(f.name + "-g", f.args,
                                         f.is_typed, f.is_negated))
                for a in self.agents:
                    pp.init.append(
                        Predicate(f.name + "-l", [a] + f.args, f.is_typed, f.is_negated))
            pp.goal = [failure] + \
                [Predicate("fin", [a], False, False) for a in self.agents]
        return pp
    ...
    def validate_adversarial(self, agent):
        ...
        pp.predicates = [infocus, failure] + [Predicate(p.name + "-l", p.args, p.is_typed, p.is_negated) for p in
                                              self.predicates] + [Predicate(p.name + "-g", p.args, p.is_typed, p.is_negated)
        ...
        for a in self.actions:
            ...
            for f in self.init:
                pp.init.append(Predicate(f.name + "-g", f.args,
                                         f.is_typed, f.is_negated))
                pp.init.append(Predicate(f.name + "-l", f.args,
                                         f.is_typed, f.is_negated))
            pp.goal = [failure] + [Predicate(p.name + "-l", p.args,
                                             p.is_typed, p.is_negated) for p in self.get_goal(agent)]
        return pp
    ...
    def validate_alone(self, agent):
        ...
        pp.type_list = self.type_list
        pp.types = self.types
        pp.goal_assignment = self.goal_assignment

        infocus = Predicate("infocus", [["?a", "agent"]], True, False)

        pp.predicates = [
            infocus] + [Predicate(p.name, p.args, p.is_typed, p.is_negated) for p in self.predicates]

        pp.actions = []
        for a in self.actions:
            a_s = Action(a.name, a.parameters,
                         [Predicate("infocus", [a.parameters.args[0][0]], False, False)] +
                         [Predicate(p.name, p.args, p.is_typed, p.is_negated)
        ...
        pp.goal = [Predicate(p.name, p.args, p.is_typed, p.is_negated)
                   for p in self.get_goal(agent)]
        ...
        # TODO ask Erez why is that needed? it causes a BUG!
    ...
    def validate_rational_waiting(self, waitfor_dict={}):
        ...
        wait_preds = filter(
            lambda x: x.name in wait_pred_names, self.predicates)
        ...
        pp.predicates = [failure, act, fin] + \
                        [Predicate(p.name + "-l", [["?alocal", "agent"]] + p.args, p.is_typed, p.is_negated) for p in
                         self.predicates] + \
                        [Predicate(p.name + "-g", p.args, p.is_typed, p.is_negated) for p in self.predicates] + \
                        [Predicate(p.name + "-checked", p.args, p.is_typed, p.is_negated) for p in self.predicates] + \
                        [Predicate(p.name + "-w", [["?alocal", "agent"]] + p.args, p.is_typed, p.is_negated) for p in
                         wait_preds]

        for p in wait_preds:
            pp.actions.append(Action("check_no_" + p.name, Predicate('', p.args, True, False),
                                     [Predicate("fin", [a], False, False) for a in self.agents] + [
                Predicate(p.name + "-g", map(lambda x: x[0], p.args), False, not p.is_negated)],
            ...
            pp.actions.append(Action("check_no_waiting_" + p.name, Predicate('', p.args, True, False),
                                     [Predicate("fin", [a], False, False) for a in self.agents] + [
                Predicate(
                    p.name + "-w", [a] + map(lambda x: x[0], p.args), False, not p.is_negated)
                for a in self.agents],
                [Predicate(p.name + "-checked", map(lambda x: x[0], p.args), False, p.is_negated)]))
        ...
        for a in self.agents:
            ...
            for i, f in enumerate(wait_preds):
                pp.actions.append(
                    Action("end_w_" + a + "_" + str(i), Predicate('', [["?alocal", "agent"]] + f.args, True, False),
                           [Predicate(p.name + "-l", [a] + p.args, p.is_typed, p.is_negated) for p in self.get_goal(a)] +
                           [Predicate(
                            f.name + "-w", [a] + map(lambda x: x[0], f.args), False, f.is_negated)],
                           [Predicate("fin", [a], False, False), Predicate("act", [], False, True),
                            Predicate("failure", [], False, False)]))
        ...
        for a in self.actions:
            ...
            for i, f in enumerate(a.precondition):
                if i in waitfor:
                    a_f.append(Action(a.name + "_w_" + str(i), a.parameters,
                                      [Predicate("act", [], False, False)] +
                                      [Predicate(p.name + "-l", [a.parameters.args[0][0]] + p.args, p.is_typed, p.is_negated) for
                                       p in a.precondition] +
                                      [Predicate(f.name + "-g", f.args,
                                                 f.is_typed, not f.is_negated)],
                                      [Predicate(p.name + "-l", [a.parameters.args[0][0]] + p.args, p.is_typed, p.is_negated) for
            ...
            for p in wait_preds:
                pp.goal += [Predicate(f.name + "-checked", f.args, False, False)
    ...
    def parse_domain(self, domainfile):
        ...
        if dfile_array[0:4] != ['(', 'define', '(', 'domain']:
            print(
                'PARSING ERROR: Expected (define (domain ... at start of domain file')
            sys.exit()
        ...
        for action in self.actions:
            ...
            agent = self._parse_name_type_pairs(
                act.get('agent'), self.type_list)
            param_list = agent + \
                self._parse_name_type_pairs(
                    act.get('parameters')[1:-1], self.type_list)
            up_params = Predicate('', param_list, True, False)
            pre_list = self._parse_unground_propositions(
                act.get('precondition'))
    ...
    def parse_problem(self, problemfile):
        ...
        if pfile_array[5:8] != [')', '(', ':domain']:
            print(
                'PARSING ERROR: Expected (:domain ...) after (define (problem ...)')
            sys.exit()
                    ...
                    if word == ')':
                        if obj_list[0] == '=' and is_function == False:
                            ...
                            if is_function:
                                ...
                                self.ground_functions.append(
                                    GroundFunction(obj_list))
                                is_function = False
                            else:
    ...
    def _parse_name_type_pairs(self, array, types):
        ...
        for i in range(0, int(len(array) / 3)):
            ...
            if array[3 * i + 2] in types:
                ...
                print("PARSING ERROR {} not in types list".format(
                    array[3 * i + 2]))
    ...
    def _parse_unground_propositions(self, array):
        """Parses possibly conjunctive list of ungrounded propositions.
...
if __name__ == "__main__":
    sys.argv = ['./ma_val.py', './benchmarks/blocksworld',
                'domain', 'probBLOCKS-9-0', 'output']
    if len(sys.argv) < 4:
        ...
        print(sys.argv)
        pp = PlanningProblem(
            sys.argv[1] + "/" + sys.argv[2] + ".pddl", sys.argv[1] + "/" + sys.argv[3] + ".pddl")
        waitfor_dict = read_waitfor(sys.argv[1] + "/waitfor.txt")

        rpp = pp.validate_rational_waiting(waitfor_dict)
        path = os.path.join(os.path.abspath(
            sys.argv[4]), "rational_wait", os.path.basename(os.path.normpath(sys.argv[1])))
        if not os.path.exists(path):
            os.makedirs(path)
        rpp.write_pddl_domain(os.path.join(
            path, "domain_" + sys.argv[3] + ".pddl"))
        rpp.write_pddl_problem(os.path.join(path, sys.argv[3] + ".pddl"))

        rpp = pp.validate_rational()
        path = os.path.join(os.path.abspath(
            sys.argv[4]), "rational_smpl", os.path.basename(os.path.normpath(sys.argv[1])))
        if not os.path.exists(path):
            os.makedirs(path)
        rpp.write_pddl_domain(os.path.join(
            path, "domain_" + sys.argv[3] + ".pddl"))
        ...
        for a in pp.agents:
            ...
            path = os.path.join(os.path.abspath(sys.argv[4]), "adversarial", os.path.basename(os.path.normpath(sys.argv[1])),
                                sys.argv[3], a)
            if not os.path.exists(path):
                os.makedirs(path)
        ...
        for a in pp.agents:
            ...
            path = os.path.join(os.path.abspath(sys.argv[4]), "alone", os.path.basename(os.path.normpath(sys.argv[1])),
                                sys.argv[3], a)
            if not os.path.exists(path):
                os.makedirs(path)
            app.write_pddl_domain(os.path.join(path, "domain.pddl"))
